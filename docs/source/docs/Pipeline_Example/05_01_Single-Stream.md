...
`dx_stream/pipelines/single_network/object_detection/run_YoloV5S_PPU.sh` and can be used as a reference for execution.
...
The pipeline in the figure is defined in 
`dx_stream/pipelines/tracking/run_multi_object_tracker.sh` and can be used as a reference for execution.
...
- **Segmentation**: Pixel-wise instance segmentation  
