...
constexpr guint FRAME_ANALYTICS_TYPE = 1001;
constexpr guint OBJECT_DETECTION_TYPE = 2001;
constexpr guint QUALITY_ASSESSMENT_TYPE = 1002;

// Release functions
static void free_frame_analytics_data(gpointer data) {
    auto *analytics = (FrameAnalyticsData *)data;
    if (analytics) {
...
static void free_object_detection_data(gpointer data) {
    auto *detection = (ObjectDetectionData *)data;
    if (detection) {
...
static void free_quality_assessment_data(gpointer data) {
    auto *quality = (QualityAssessmentData *)data;
    if (quality) {
...
static gpointer copy_frame_analytics_data(gpointer data) {
    const auto *src = (FrameAnalyticsData *)data;
    if (!src) return nullptr;
    
    auto *dst = g_new0(FrameAnalyticsData, 1);
...
static gpointer copy_object_detection_data(gpointer data) {
    const auto *src = (ObjectDetectionData *)data;
    if (!src) return nullptr;
    
    auto *dst = g_new0(ObjectDetectionData, 1);
    dst->custom_type = src->custom_type;
...
static gpointer copy_quality_assessment_data(gpointer data) {
    const auto *src = (QualityAssessmentData *)data;
    if (!src) return nullptr;
    
    auto *dst = g_new0(QualityAssessmentData, 1);
    dst->custom_type = src->custom_type;
...
static const char* get_random_scene_type() {
    static const std::array<const char*, 6> scenes = {"indoor", "outdoor", "street", "office", "home", "park"};
    return scenes[g_random_int_range(0, 6)];
}

static const char* get_random_object_type() {
    static const std::array<const char*, 7> objects = {"person", "car", "bike", "dog", "cat", "truck", "bus"};
...
static GstPadProbeReturn add_metadata_probe(GstPad *pad, GstPadProbeInfo *info, gpointer user_data) {
    ...
    if (!frame_meta) {
        ...
        frame_meta = dx_get_frame_meta(buffer);
        g_print("  > Created new DXFrameMeta\n");
    }
    
    // === Add Frame Analytics Data ===
    auto *frame_analytics = g_new0(FrameAnalyticsData, 1);
    frame_analytics->custom_type = FRAME_ANALYTICS_TYPE;
    frame_analytics->scene_type = g_strdup(get_random_scene_type());
    frame_analytics->brightness_level = g_random_double(); // NOSONAR: g_random_double returns 0.0 - 1.0
    frame_analytics->motion_level = g_random_double(); // NOSONAR: g_random_double returns 0.0 - 1.0
    ...
    auto *quality_data = g_new0(QualityAssessmentData, 1);
    quality_data->custom_type = QUALITY_ASSESSMENT_TYPE;
    quality_data->overall_quality = g_random_double(); // NOSONAR: g_random_double returns 0.0 - 1.0
    quality_data->sharpness = g_random_double(); // NOSONAR: g_random_double returns 0.0 - 1.0
    quality_data->exposure = g_random_double(); // NOSONAR: g_random_double returns 0.0 - 1.0
    ...
    if (dx_user_meta_set_data(quality_user_meta, quality_data, sizeof(QualityAssessmentData),
                             DXUserMetaType::DX_USER_META_FRAME, free_quality_assessment_data, copy_quality_assessment_data)) {
    ...
    for (int i = 0; i < frame_analytics->detected_objects; i++) {
        ...
        obj_meta->_box[0] = g_random_double_range(0.0, 800.0); // NOSONAR: g_random_double_range returns value in specified range
        obj_meta->_box[1] = g_random_double_range(0.0, 800.0); // NOSONAR: g_random_double_range returns value in specified range
        obj_meta->_box[2] = g_random_double_range(50.0, 250.0); // NOSONAR: g_random_double_range returns value in specified range
        obj_meta->_box[3] = g_random_double_range(50.0, 250.0); // NOSONAR: g_random_double_range returns value in specified range
        
        // Add object detection user metadata
        auto *obj_detection = g_new0(ObjectDetectionData, 1);
        obj_detection->custom_type = OBJECT_DETECTION_TYPE;
        obj_detection->object_type = g_strdup(get_random_object_type());
        obj_detection->detection_confidence = 0.5 + g_random_double() * 0.5; // NOSONAR: g_random_double returns 0.0 - 1.0, so this gives 0.5 - 1.0 confidence
        obj_detection->quality_score = g_random_double(); // NOSONAR: g_random_double returns 0.0 - 1.0
        ...
        if (dx_user_meta_set_data(obj_user_meta, obj_detection, sizeof(ObjectDetectionData),
                                 DXUserMetaType::DX_USER_META_OBJECT, free_object_detection_data, copy_object_detection_data)) {
...
static GstPadProbeReturn read_metadata_probe(GstPad *pad, GstPadProbeInfo *info, gpointer user_data) {
    ...
    auto frame_user_metas = dx_get_frame_user_metas(frame_meta);
    g_print("  > Found %zu frame user metadata entries\n", frame_user_metas->size());
    
    for (auto user_meta : *frame_user_metas) {
        
        if (user_meta->user_meta_data) {
            const auto *custom_type = (guint *)user_meta->user_meta_data;
            
            switch (*custom_type) {
                case FRAME_ANALYTICS_TYPE: {
                    auto *analytics = (FrameAnalyticsData *)user_meta->user_meta_data;
    ...
    for (auto obj_meta : frame_meta->_object_meta_list) {
        ...
        auto obj_user_metas = dx_get_object_user_metas(obj_meta);
        for (auto obj_user_meta : *obj_user_metas) {
            
            if (!obj_user_meta->user_meta_data) {
                continue;
            }
            const auto *custom_type = (guint *)obj_user_meta->user_meta_data;
                
            if (*custom_type == OBJECT_DETECTION_TYPE) {
                auto *detection = (ObjectDetectionData *)obj_user_meta->user_meta_data;
...
static void on_pad_added(GstElement *element, GstPad *pad, gpointer data)
{
    ...
    if (g_str_has_prefix(name, "video") && gst_pad_can_link(pad, sinkpad)) {
