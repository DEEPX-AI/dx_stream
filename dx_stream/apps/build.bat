@echo off
REM ============================================================
REM  DX-Stream Apps Build Script (Windows)
REM  Builds example applications (mqtt, kafka, usermeta).
REM  Usage:
REM    build.bat              -> build all apps
...
REM ============================================================
REM  Subroutine: build a single app
REM ============================================================
:build_one
set "APP_DIR=%~1"
set "APP_NAME=%~nx1"
set "APP_BUILD=%APP_DIR%\builddir"
...
pushd "%APP_DIR%" || (echo [FAIL] %APP_NAME%: cannot enter directory & set /a FAIL_COUNT+=1 & goto :eof)

if not exist "builddir\build.ninja" (
    meson setup builddir --buildtype=%BUILD_TYPE% >nul 2>&1
    if errorlevel 1 (
        echo   [SKIP] %APP_NAME%: meson setup failed
        popd
        set /a FAIL_COUNT+=1
        goto :eof
    )
...
if errorlevel 1 (
    echo   [FAIL] %APP_NAME%: compile failed
    popd
    set /a FAIL_COUNT+=1
    goto :eof
)
