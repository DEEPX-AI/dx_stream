...
for INPUT_VIDEO_PATH in "${INPUT_VIDEO_PATH_LIST[@]}"; do
    gst-launch-1.0 urisourcebin uri=file://$INPUT_VIDEO_PATH ! $DECODE_PIPELINE ! \
                    dxpreprocess config-file-path=$SRC_DIR/configs/Face_Detection/SCRFD500M_PPU/preprocess_config.json ! queue ! \
                    dxinfer config-file-path=$SRC_DIR/configs/Face_Detection/SCRFD500M_PPU/inference_config.json ! queue ! \
                    dxpostprocess config-file-path=$SRC_DIR/configs/Face_Detection/SCRFD500M_PPU/postprocess_config.json ! queue ! \
