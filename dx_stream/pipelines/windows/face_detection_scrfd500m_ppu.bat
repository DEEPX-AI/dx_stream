@echo off
setlocal
title DX-Stream Face Detection (SCRFD500M PPU)
...
set "MODEL=%MODEL_DIR%/SCRFD500M_PPU.dxnn"
...
echo ============================================================
echo  DX-Stream Face Detection (SCRFD500M PPU)
echo ============================================================
echo  Model : %MODEL%
echo  Video : %VIDEO%
echo ============================================================
...
gst-launch-1.0 ^
    ...
    ! queue max-size-buffers=1 ^
    ! dxpostprocess inference-id=1 library-file-path="%LIB%" function-name=SCRFD500M_PPU ^
    ! dxosd ^
    ! videoconvert ^
