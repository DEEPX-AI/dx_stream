...
std::vector<BoundingBox> parse_multi_output(const std::vector<dxs::DXTensor>& outputs,
    ...
    // Tensor nsmes
    std::vector<std::string> tensor_names = {"332", "393", "454"};
 ...
 * 
 * 1. Single Output: Looks for "515" blob (ONNX converted models)
...
extern "C" void PostProcess(GstBuffer *buf,
    ...
    int ort_idx = get_index_by_tensor_name(network_output, "515");
