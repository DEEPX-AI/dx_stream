...
std::vector<BoundingBox> parse_multi_output(const std::vector<dxs::DXTensor>& outputs,
    ...
    // Tensor nsmes
    std::vector<std::string> tensor_names = {"611", "903", "1195"};
 ...
 * 
 * 1. Single Output: Looks for "1487" blob (ONNX converted models)
...
extern "C" void PostProcess(GstBuffer *buf,
    ...
    int ort_idx = get_index_by_tensor_name(network_output, "1487");
