...
REQUIRED_FILES = {
    ...
    "skills": [
        "dx-agent-stream-build-pipeline/SKILL.md",
        "dx-agent-stream-build-mqtt-kafka/SKILL.md",
        "dx-agent-stream-model-management/SKILL.md",
        "dx-agent-stream-validate/SKILL.md",
    ],
    "toolsets": [
...
def check_content_quality(deepx_root: str, result: FrameworkValidation):
    ...
    for root, dirs, files in os.walk(deepx_root):
        for f in files:
            ...
            # Check for Korean text (exclude README-KO and intentional
            # Response Language examples like "한글 음차 표기 금지",
            # "모델, 서버, 파일, 데이터")
            korean_chars = []
            if not rel_path.endswith("-KO.md"):
...
def check_skill_structure(deepx_root: str, result: FrameworkValidation):
    ...
    # Process skills (SWE workflow helpers: brainstorm, tdd, verify, etc.) follow
    # their own Gate Function / Iron Law structure and are exempt from the
    # required_headings check. Stream-specific skills (dx-agent-stream-*) retain
    # the standard headings validation.
    process_skills = {
        "dx-agent-brainstorm",
        "dx-agent-tdd",
        "dx-agent-verify",
    ...
    for skill_file in sorted(Path(skills_dir).glob("*/SKILL.md")):
        ...
        if rel_name in process_skills:
            result.add("skill-structure", f"skill:{rel_name}",
                       True, f"Skill structure OK (process skill): {rel_name}")
            continue
        ...
        if missing:
            result.add("skill-structure", f"skill:{rel_name}",
                       False,
                       f"Missing section(s) in {rel_name}: "
                       f"{', '.join(missing)}")
        else:
            result.add("skill-structure", f"skill:{rel_name}",
                       True, f"Skill structure OK: {rel_name}")
