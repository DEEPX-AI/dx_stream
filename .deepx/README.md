...
| `/dx-agent-brainstorm` | Brainstorm and plan before any code generation (process skill) |
| `/dx-agent-tdd` | Test-driven development — validate each file immediately after creation (process skill) |
| `/dx-agent-verify` | Verify before claiming completion — evidence before assertions, mandatory artifacts enforcement (process skill) |
...
.deepx/
  ...
  skills/
    dx-agent-stream-build-pipeline/
      SKILL.md                           # Full pipeline build skill
    dx-agent-stream-build-mqtt-kafka/
      SKILL.md                           # Broker integration skill
    dx-agent-stream-model-management/
      SKILL.md                           # Model management skill
...
This knowledge base supports five AI coding tools:
