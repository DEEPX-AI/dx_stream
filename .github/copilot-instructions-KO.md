...
15. **객체 감지 파이프라인에서 DxTracker 필수**: 객체 감지 모델(yolo*, ssd, scrfd 등)을 위한 파이프라인은 반드시 `dxpostprocess`와 `dxosd` 사이에 `dxtracker config-file-path=../../dx_stream/configs/tracker_config.json`를 포함해야 한다. 이 경로를 기본값으로 사용하고 tracker를 조건부로 만들거나 감지 파이프라인에서 생략하지 않는다.
