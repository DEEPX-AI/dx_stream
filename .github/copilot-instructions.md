...
| Skill | Description |
|-------|-------------|
| dx-agentic-stream-build-pipeline | Build GStreamer pipeline (single, multi, cascaded, tiled, parallel, broker) |
| dx-agentic-stream-build-mqtt-kafka | Build MQTT/Kafka message broker pipeline |
| dx-agentic-stream-model-management | Download and configure .dxnn models for pipelines |
...
15. **DxTracker mandatory for detection**: Any pipeline for an object-detection model (yolo*, ssd, scrfd, etc.) MUST include `dxtracker config-file-path=../../dx_stream/configs/tracker_config.json` between `dxpostprocess` and `dxosd`. Always use this resolved path as the default — never make the tracker conditional or omit it from detection pipelines.
