...
The pipeline app dir MUST run when **copied OUTSIDE the suite** (only the DEEPX runtime is
external). Copy it to a temp dir outside the suite and re-check:

```bash
APP="<app_dir>"; TMP="$(mktemp -d /tmp/dx_portable.XXXXXX)"
rsync -a --exclude venv --exclude __pycache__ --exclude output "$APP"/ "$TMP/app/"; cd "$TMP/app"
find . -type l | while read -r l; do t=$(readlink -f "$l"); case "$t" in "$PWD"/*) ;; *) echo "FAIL escaping-symlink: $l -> $t";; esac; done
grep -rn "dx-agent-dev-showcase/\|/dx-all-suite/" --include='*.py' --include='*.sh' . | grep -v '/sample/' \
  && echo "FAIL: source reference" || echo "OK: no source refs"
cd - >/dev/null; rm -rf "$TMP"
```
**FAIL** if a symlink escapes the app, or a `dx-agent-dev-showcase/`/suite **code** reference
exists (the model-path must resolve via `$SUITE_ROOT` auto-detect, NOT a hardcoded suite path;
input `/sample/` media is allowed). See the suite-level **Self-Contained & Portable** gate.
...
**Session:** dx-agent-dev/<session_dir>/
