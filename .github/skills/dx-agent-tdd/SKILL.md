---
name: dx-agent-tdd
description: Test-driven development for dx_stream
---
...
Active during any pipeline build (`/dx-agent-stream-build-pipeline`, `/dx-agent-stream-build-mqtt-kafka`).
...
Run each check from the session directory (e.g., `dx-agent-dev/<session>/`).
