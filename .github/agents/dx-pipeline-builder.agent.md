...
description: Build GStreamer dx_stream pipeline applications with proper element
  configuration, preprocess-id matching, and queue placement.
argument-hint: e.g., single-model detection pipeline
tools:
...
## Critical Rules
- preprocess-id matching between DxPreprocess and DxInfer
- Queue between every processing stage
...
|---|---|---|
| 0 | Run `sanity_check.sh --dx_rt` and `gst-inspect-1.0 dxinfer` | FAIL → `install.sh --all --exclude-app --exclude-stream --skip-uninstall --venv-reuse` → re-run sanity_check → **STOP if still failing (unconditional — user cannot override).** If NPU hardware init failure → guide user to cold boot/reboot, then STOP |
