...
bool LibyuvTransformKernel::scale_i420(
    ...
    int src_w, int src_h,
    uint8_t* dst, int dst_w, int dst_h) const
{
    uint8_t* dst_y = dst;
    ...
    uint8_t* dst, int dst_w, int dst_h) const
{
    ...
    int ret = libyuv::NV12Scale(
        ...
        dst_y, dst_w, dst_uv, dst_w, dst_w, dst_h,
...
TransformResult LibyuvTransformKernel::transform(const FrameDesc&  src,
        ...
        if (!needs_scale) {
            ...
            if (bpp > 0) {
                ...
                scale_i420(cr_y, cr_stride_y, cr_u, cr_stride_u, cr_v, cr_stride_v,
                           crop_w, crop_h, dst_content_ptr, content_w, content_h);
            } else if (dst_fmt == VideoFormat::NV12) {
    ...
    // Case B: Convert only (no scale)
        ...
        result.success = convert_color(
            ...
            dst_content_ptr, dst_content_stride,
            dst.num_planes > 1 ? dst.planes[1].data : nullptr,
            dst.num_planes > 1 ? dst.planes[1].stride : 0,
            dst.num_planes > 2 ? dst.planes[2].data : nullptr,
            dst.num_planes > 2 ? dst.planes[2].stride : 0,
            dst_fmt);
    }
    // Case C: Scale + Convert (needs 1 scratch buffer)
        ...
        if (src_fmt == VideoFormat::I420) {
            scratch_size = content_w * content_h * 3 / 2;
        } else if (src_fmt == VideoFormat::NV12) {
            scratch_size = content_w * content_h * 3 / 2;
        } else {
        ...
        if (src_fmt == VideoFormat::I420) {
            ...
            s_stride_u = content_w / 2;
            s_stride_v = content_w / 2;
            s_u = s_y + content_w * content_h;
            s_v = s_u + (content_w / 2) * (content_h / 2);
        } else if (src_fmt == VideoFormat::NV12) {
            s_stride_y = content_w;
