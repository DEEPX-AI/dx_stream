...
static GstFlowReturn gst_dxscale_transform(GstBaseTransform *trans,
...

    {
        ...
        if (gst_fmt == GST_VIDEO_FORMAT_NV12) {
            // Input: pass GstVideoInfo so CPU path uses authoritative stride/offset
            // (fixes x86 SW-decoded buffers); DMA-buf path is unaffected.
            src_desc = dxt::make_nv12_frame_desc(inbuf, in_w, in_h, &self->_input_info);
            // Force CPU path — we already mapped the buffer
