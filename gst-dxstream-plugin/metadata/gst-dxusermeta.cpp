...
DXUserMeta* dx_acquire_user_meta_from_pool(void) {
    GST_CAT_DEBUG_SAFE(dxmeta_cat, "Acquiring DXUserMeta from pool");
...
void dx_release_user_meta(DXUserMeta *user_meta) {
    GST_CAT_DEBUG_SAFE(dxmeta_cat, "Releasing DXUserMeta");
    if (!user_meta) return;
    ...
    if (user_meta->user_meta_data) {
        if (user_meta->release_func) {
            ...
            GST_CAT_WARNING_SAFE(dxmeta_cat, "No release_func set for user metadata - potential memory leak!");
        }
    }
...
gboolean dx_user_meta_set_data(DXUserMeta *user_meta,
    ...
    GST_CAT_DEBUG_SAFE(dxmeta_cat, "Setting data for DXUserMeta of type %d", static_cast<int>(meta_type));
    if (!user_meta) return FALSE;
    
    if (!release_func || !copy_func) {
        GST_CAT_WARNING_SAFE(dxmeta_cat, "Both release_func and copy_func are required for user metadata");
        return FALSE;
    }
...
gboolean dx_add_user_meta_to_frame(DXFrameMeta *frame_meta, DXUserMeta *user_meta) {
    GST_CAT_DEBUG_SAFE(dxmeta_cat, "Adding DXUserMeta to frame");
    if (!frame_meta || !user_meta) {
        return FALSE;
    }
    
    if (!user_meta->release_func || !user_meta->copy_func) {
        GST_CAT_WARNING_SAFE(dxmeta_cat, "DXUserMeta must have both release_func and copy_func set before adding to frame");
        return FALSE;
    }
...
gboolean dx_add_user_meta_to_obj(DXObjectMeta *obj_meta, DXUserMeta *user_meta) {
    GST_CAT_DEBUG_SAFE(dxmeta_cat, "Adding DXUserMeta to object");
    if (!obj_meta || !user_meta) {
        return FALSE;
    }
    
    if (!user_meta->release_func || !user_meta->copy_func) {
        GST_CAT_WARNING_SAFE(dxmeta_cat, "DXUserMeta must have both release_func and copy_func set before adding to object");
        return FALSE;
    }
...
std::vector<DXUserMeta*>* dx_get_frame_user_metas(DXFrameMeta *frame_meta) {
    GST_CAT_DEBUG_SAFE(dxmeta_cat, "Getting user metas from frame");
...
std::vector<DXUserMeta*>* dx_get_object_user_metas(DXObjectMeta *obj_meta) {
    GST_CAT_DEBUG_SAFE(dxmeta_cat, "Getting user metas from object");
