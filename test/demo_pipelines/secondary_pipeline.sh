...
OUTPUT=$(gst-launch-1.0 urisourcebin uri=file://$VIDEO_FILE ! decodebin ! \
    ...
    dxpreprocess config-file-path=$SRC_DIR/dx_stream/configs/Face_Detection/SCRFD/preprocess_config.json ! queue ! \
    dxinfer config-file-path=$SRC_DIR/dx_stream/configs/Face_Detection/SCRFD/inference_config.json ! queue ! \
    dxpostprocess config-file-path=$SRC_DIR/dx_stream/configs/Face_Detection/SCRFD/postprocess_config.json ! queue ! \
    dxosd ! \
    fakesink 2>&1)
...
OUTPUT=$(gst-launch-1.0 urisourcebin uri=file://$VIDEO_FILE ! decodebin ! \
    ...
    tee name=t \
    t. ! queue ! \
    dxpreprocess config-file-path=$SRC_DIR/dx_stream/configs/Re-Identification/OSNet/preprocess_config.json ! queue ! \
    dxinfer config-file-path=$SRC_DIR/dx_stream/configs/Re-Identification/OSNet/inference_config.json ! queue ! \
    dxpostprocess config-file-path=$SRC_DIR/dx_stream/configs/Re-Identification/OSNet/postprocess_config.json ! queue ! \
    gather.sink_0 \
    t. ! queue ! \
    dxpreprocess config-file-path=$SRC_DIR/dx_stream/configs/Face_Detection/SCRFD/preprocess_config.json ! queue ! \
    dxinfer config-file-path=$SRC_DIR/dx_stream/configs/Face_Detection/SCRFD/inference_config.json ! queue ! \
