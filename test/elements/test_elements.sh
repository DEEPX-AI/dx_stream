...
g++ -g -o "$SCRIPT_DIR/bin/test_dxscale" test_dxscale.cpp $(pkg-config --cflags --libs gstreamer-check-1.0 gstreamer-app-1.0 gstdxstream opencv4) -ldxrt
if [ $? -ne 0 ]; then
    echo "[ERROR] Failed to compile test_dxscale" >&2
    exit 1
fi

g++ -g -o "$SCRIPT_DIR/bin/test_dxconvert" test_dxconvert.cpp $(pkg-config --cflags --libs gstreamer-check-1.0 gstreamer-app-1.0 gstdxstream opencv4) -ldxrt
if [ $? -ne 0 ]; then
    echo "[ERROR] Failed to compile test_dxconvert" >&2
    exit 1
fi

# Kernel test needs internal headers — add source-tree include path
PLUGIN_SRC="$SCRIPT_DIR/../../gst-dxstream-plugin/src"
g++ -g -o "$SCRIPT_DIR/bin/test_libyuv_kernel" test_libyuv_kernel.cpp \
    -I"$PLUGIN_SRC" \
    $(pkg-config --cflags --libs gstreamer-check-1.0 gstdxstream) -ldxrt
if [ $? -ne 0 ]; then
    echo "[ERROR] Failed to compile test_libyuv_kernel" >&2
    exit 1
fi
